...
const ERP_Purchase_InsertPage = () => {
    ...
    const onClickSaleInsert = async () => {
        ...
        purchase_id && await Promise.all(items.map(item => axios.post(`/erp/purchase/info`, {
            ...item, purchase_id,
                ...
                <Col lg={12}>
                    ...
                    <Card>
                        <Card.Header>
                            ...
                            <Row>
                                <Col lg={1}>
                                    <div className='align-middle mt-2'>담당자:</div>
                                </Col>
                                <Col lg={11}>
                                    <Form.Select value={master.purchase_employee} name='purchase_employee' onChange={onChageMaster}>
                                        <option >담당자를선택하세요</option>
                                        {memberList && memberList.map(mem => mem.dept_name.includes("경영") && mem.member_info_job !== '퇴사' &&
